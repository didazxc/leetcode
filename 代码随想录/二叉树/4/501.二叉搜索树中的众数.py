

class TreeNode:
    def __init__(self, val, left=None, right=None):
        self.val = val
        self.left = left
        self.right = right

    def get_lines(self):
        ls, llen, _, lpad = self.left.get_lines() if self.left else ([], 0, 0, 0)
        rs, rlen, rpad, _ = self.right.get_lines() if self.right else ([], 0, 0, 0)
        line_num = max(len(ls), len(rs))
        ls += [' ' * llen] * (line_num - len(ls))
        rs += [' ' * rlen] * (line_num - len(rs))
        val_len = len(str(self.val))
        lines = [ls[i] + ' ' * (val_len + 2) + rs[i] for i in range(line_num)]
        first_line = [f'{" "*llen} {self.val} {" "*rlen}']
        second_line = [(lpad if llen else ' '*llen) + ' ' * (val_len+2) + (rpad if rlen else ' '*rlen)]
        length = llen + val_len + 2 + rlen
        return first_line + second_line + lines, length, ' '*llen + '\\' + ' '*(length-llen-1), ' '*(llen+1+val_len)+'/'+' '*rlen

    def __str__(self):
        lines, *_ = self.get_lines()
        return '\n'.join(lines)

    @staticmethod
    def apply(nums):
        n = len(nums)

        def build(i):
            left = build(2*i+1) if 2*i+1 < n else None
            right = build(2*i+2) if 2*i+2 < n else None
            return TreeNode(nums[i], left=left, right=right) if nums[i] else None

        return build(0)


def find0(root: TreeNode):
    cur = root
    pre = None
    max_count = 1
    count = 1
    res = []
    stack = []
    while cur or stack:
        if cur:
            stack.append(cur)
            cur = cur.left
        else:
            cur = stack.pop()
            if pre is None:
                count = 1
            elif cur.val == pre.val:
                count += 1
            else:
                count = 1

            if count == max_count:
                res.append(cur.val)
            elif count > max_count:
                res = [cur.val]
                max_count = count

            pre = cur
            cur = cur.right
    return res


def find(root: TreeNode):
    res = []
    count: dict[str, int or TreeNode] = {'pre': None, 'count': 1, 'max_count': 1}

    def fn(node: TreeNode):
        if node is None:
            return
        fn(node.left)
        if count['pre'] is None:
            count['count'] = 1
        elif count['pre'].val == node.val:
            count['count'] += 1
        else:
            count['count'] = 1
        if count['count'] == count['max_count']:
            res.append(node.val)
        elif count['count'] > count['max_count']:
            res[:] = [node.val]
            count['max_count'] = count['count']
        count['pre'] = node
        fn(node.right)
    fn(root)
    return res


if __name__ == '__main__':
    r = TreeNode.apply([4, 2, 5, 1, 3, 5])
    print(r)
    print(find(r))
